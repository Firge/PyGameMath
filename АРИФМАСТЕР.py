...
def load_image(gamemode, name):
    ...
    print(os.path.abspath(f'data\\{gamemode}\\img'))
...
class Profil:
    def __init__(self, screen):
        ...
        self.progress_bar = pg.transform.scale(self.progress_bar, (292, 52))
        self.edit = load_image('Profile', "edit.png")
        self.edit = pg.transform.scale(self.edit, (61, 61))
        self.books = load_image('Profile', "books.png")
        self.books = pg.transform.scale(self.books, (71, 61))
        self.asteroid = load_image('Profile', "asteroid.png")
        self.asteroid = pg.transform.scale(self.asteroid, (71, 71))
        self.lily = load_image('Profile', "lily.png")
        self.lily = pg.transform.scale(self.lily, (70, 70))
        self.lily = pg.transform.rotate(self.lily, 90)
        self.star = pg.transform.scale(self.star, (211, 171))
        self.photo = pg.transform.scale(self.photo, (181, 171))
        self.frame = pg.transform.scale(self.frame, (181, 171))
        n = 1.35
        self.desk = pg.transform.scale(self.desk, (
            int(self.desk.get_width() * n), int(self.desk.get_height() * n)))
        # background = pg.transform.rotate(background, 90)
        # background = pg.transform.scale(background, (int(650 * 1.777777777), int(650)))
    ...
    def start(self, x):
        ...
        ButtonBacktoMenu.draw(self.screen)
        n = 37
        ny = 26
        pg.draw.polygon(self.screen, (255, 255, 255), [[280 - n, 510 - ny],
                                                       [571 - n, 510 - ny],
                                                       [571 - n, 561 - ny],
                                                       [280 - n, 561 - ny]])
        if x:
            pg.draw.rect(self.screen, (244, 191, 0), [243, 484, 1 + 291 * x, 52])
...
def loads():
    ...
    k = x // 980
    topy = 650 - y // k
    new_record = pg.transform.scale(new_record,
                                    (new_record.get_width() * 2, new_record.get_height() * 2))
    background = pg.transform.scale(background, (int(650 * 1.777777777), y // k))
    background = pg.transform.rotate(background, 180)

    moon_pct = load_image('AsteroidRain', "luna.png")
    moon_pct = pg.transform.rotate(moon_pct, -90)

    hart = load_image('AsteroidRain', "qqq.png")
    hart = pg.transform.scale(hart, (60, 60))

    boom_pct = load_image('AsteroidRain', "bum.png")
    boom_pct = pg.transform.scale(boom_pct, (int(171 * 1.5), int(171 * 1.5)))

    asteroid_pct = load_image('AsteroidRain', "asteroid_final.png")

    line = load_image('AsteroidRain', "line.png")
    line = pg.transform.scale(line, (int(1735 // 2), int(144 // 2)))
    line = pg.transform.rotate(line, 90)
    return line, background, topy, moon_pct, hart, boom_pct, asteroid_pct, asteroid_pct, panel, prestart, smail, but_star, new_record


def how_num(std, x, y):
    if 220 <= x <= 1101 and 440 <= y <= 600 and std.score == 11:
        return 101

    if 0 <= x - 50 * (x // 220 + 1) - 170 * (x // 220):
        if 50 <= y <= 210 and std.score >= x // 220 + 1:
            return x // 220 + 1
        if 250 <= y <= 420 and std.score >= x // 220 + 6:
            return x // 220 + 6
        if 440 <= y <= 600:
            return 11
...
def blit_otv(screen, box_otv):
    font = pg.font.Font("data/Menu/font/norm.ttf", 120)
...
class study:
    def __init__(std, screen, user_id, vol0, vol1):
        ...
        std.vol1 = vol1
        std.user_id = user_id
        # std.screen = pg.display.set_mode((int(650 * 1.777777777), 650))
        std.screen = screen
        std.TEST = False
        std.MENU = True
        std.MAIN = True
        ...
        # std.menu_pct = pg.transform.scale(std.menu_pct, [1155, 650])
    ...
    def main(std):
        while std.MAIN:
            ...
            if std.MENU:
                std.score = sorted(get_user_best_score(std.user_id))[0][1]
                std.menu()
                std.blit_block()
            elif std.TUTORIAL:
    ...
    def tutorial(std, num):
        ...
        font = pg.font.Font("data/Menu/font/norm.ttf", 70)
        for i in range(5):
            for j in range(2):
    ...
    def blit_table0(std, num):
        font = pg.font.Font("data/Menu/font/norm.ttf", 60)
        for i in range(5):
            for j in range(2):
                color = (255, 70, 80)
                ...
                if std.variable == 101:
                    text = font.render(
                        f"{std.box[i * 2 + j].replace(' ', '')}={eval(std.box[i * 2 + j].replace('x', '*'))}",
                        True,
                        color)
                    pass
                else:
        ...
        font = pg.font.Font("data/Test/font/ariblk.ttf", 30)
        ...
        pg.draw.circle(std.screen, (75, 179, 75), [1070, h // 2 + 460], 20)
        pg.draw.circle(std.screen, (255, 70, 80), [1070, h // 2 + 510], 20)
...
def get_user_best_score(user_id):
    score = cur.execute('SELECT game_id, score FROM opened_games WHERE user_id = ?',
                        (user_id,)).fetchall()
    score.sort()
    return score
...
class Lily(pg.sprite.Sprite):
    image = pg.transform.scale(load_image('MrFrog', 'lily.png'),
                               (int(size_x * 0.14), int(size_y * 0.21)))
    image_drown = pg.transform.scale(load_image('MrFrog', 'boom.png'),
                                     (int(size_x * 0.14), int(size_y * 0.21)))
    ...
    def __init__(self, x, y, text, par):
        ...
        self.rect.y = y
        self.num = int(text)
        font = pg.font.Font(os.path.join('data/MrFrog/font', 'font.ttf'), 48)
        string_rendered = font.render(text, True, pg.Color('blue'))
        self.image.blit(string_rendered, (self.image.get_width() // 3, self.image.get_height() // 2))

    def collide(self, pos):
        mask = pg.mask.from_surface(pg.surface.Surface((1, 1)))
    ...
    def update(self, *args):
        ...
        elif args and args[0].type == pg.MOUSEBUTTONDOWN and self.collide(args[0].pos):
...
class FrogGame:
    def __init__(self, screen, user_id, vol1, vol0):
        ...
        self.win = pg.mixer.Sound(os.path.join('data/MrFrog/music', 'win.wav'))
        self.win.set_volume(vol0)
        self.lose = pg.mixer.Sound(os.path.join('data/MrFrog/music', 'lose.wav'))
        self.lose.set_volume(vol0)
        self.bul = pg.mixer.Sound(os.path.join('data/MrFrog/music', 'bul.wav'))
        self.bul.set_volume(vol0)
        self.jump = pg.mixer.Sound(os.path.join('data/MrFrog/music', 'jump.wav'))
        self.jump.set_volume(vol0)
        self.background = pg.transform.scale(load_image('MrFrog', 'water_bresk.jpeg'),
                                             (size_x, size_y))
        self.frog = pg.transform.scale(
            pg.transform.rotate(load_image('MrFrog', 'frog.png'), -90),
            (int(0.08 * size_x), int(0.13 * size_y)))
        self.frog = pg.transform.rotate(self.frog, 90)
        self.frog_jump = pg.transform.scale(load_image('MrFrog', 'frog_jump.png'),
                                            (int(0.13 * size_x), int(0.15 * size_y)))
        self.frog_jump = pg.transform.rotate(self.frog_jump, 90)
        self.exit = pg.transform.scale(load_image('MrFrog', 'exit.png'),
                                       (50, 50))
        self.all_score = 0
        self.running = True
        self.waiting = False
    ...
    def starting_event(self):
        ...
        self.lily_group = pg.sprite.Group()
        self.lily_group.add(
            Lily(int(0.62 * size_x), size_y // 4 - int(size_y / 4.8) // 2, str(var[0]), self))
        self.lily_group.add(
            Lily(int(0.72 * size_x), size_y // 2 - int(size_y / 4.8) // 2, str(var[1]), self))
        self.lily_group.add(
            Lily(int(0.62 * size_x), size_y // 4 * 3 - int(size_y / 4.8) // 2, str(var[2]), self))
        for lily in self.lily_group:
            if self.solution_type:
                ...
                if self.solution[0] - self.solution[1] == lily.num:
                    ...
                    break
        font = pg.font.Font(os.path.join('data/MrFrog/font', 'font.ttf'), 48)
        self.score_out = font.render(f'Счёт: {self.score}', False, pg.color.Color('blue'))
        best_score = get_user_best_score(self.user_id)
        best_score.sort()
        best_score = best_score[1]
        self.record = best_score[1]
        self.record_out = font.render(f'Лучший: {self.record}', False, pg.color.Color('blue'))
        self.clock.tick()
        self.starting = False
    ...
    def waiting_event(self):
        ...
        self.screen.fill((255, 0, 0), (
            self.timer_size[0] + 3, 23, (self.timer_size[2] - 6) * self.n // self.time, 14))
        ...
        string_rendered = font.render(f"{self.solution[0]} {'+' if self.solution_type else '-'} "
                                      f"{self.solution[1]}", True, pg.Color('blue'))
    ...
    def ending_event(self):
        ...
        self.screen.blit(self.start_lily, self.start_lily_coords)
        self.lily_group.draw(self.screen)
        self.screen.blit(pg.transform.rotate(self.frog_jump, self.angle),
...
def colis(i, event, topy):
    ...
    if x11 is not None or x12 is not None:
...
class Game0:
    ...
    def blit_asteroid(self, bang, screen):
        font = pg.font.Font("data/AsteroidRain/font/Prime-Regular.ttf", 70)
        tok = 0
    ...
    def game_over(self, screen):
        screen_over = pg.Surface(screen.get_size())
    ...
    def draw_pistol(self, screen, cos_a, sin_a, color, timer):
        ...
        height = h = 150
        topy = 68
        pg.draw.polygon(screen, (125, 125, 125),
                        [[(100 + h * cos_a) + w * sin_a,
        ...
        height = h = 150
        height = h1 = h + 20
        pg.draw.polygon(screen, (125, 0, 125),
                        [[(100 + h * cos_a) + w * sin_a,
                          (topy + (650 - topy) // 2 + h * sin_a) - w * cos_a],
                         [(100 + h * cos_a) - w * sin_a,
                          (topy + (650 - topy) // 2 + h * sin_a) + w * cos_a],
        ...
        height = h = h1
        height = h1 = h + 10
        pg.draw.polygon(screen, (125, 100, 0),
                        [[(100 + h * cos_a) + w * sin_a,
                          (topy + (650 - topy) // 2 + h * sin_a) - w * cos_a],
                         [(100 + h * cos_a) - w * sin_a,
                          (topy + (650 - topy) // 2 + h * sin_a) + w * cos_a],
        ...
        pg.draw.circle(screen, (125, 125, 0), (100, topy + (650 - topy) // 2), 60, 0)
    ...
    def __init__(self, screen, user_id, vol0, vol1):
        ...
        # screen = pg.display.set_mode((int(650 * 1.777777777), 650))
        screen = screen
        clock = pg.time.Clock()
        prestart = True
        start = False
        ...
        while game:
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    ...
                    pg.quit()
                    sys.exit()
                if event.type == pg.MOUSEMOTION and event.pos[1] > 68:
                    qx, qy = event.pos
                    if qx <= 100:
                        cos_a = 0
                        if qy > 359:
                ...
                if start and event.type == pg.MOUSEBUTTONDOWN:
                    win.play(0)
                    clock.tick(100)
                    timer_color = 1
                    x, y = event.pos
                            ...
                            if p != None:
                                ...
                                if self.list_otv[0][num] == eval(self.primers[0]):
                                    ...
                                    self.box_of_error.append(
                                        (self.primers[0], self.list_otv[0][num],
                                         eval(self.primers[0])))
                                    health_point -= 1
                                    if net_tok % 2:
                                        box_net[0].play(0)
                ...
                if not start and not prestart and event.type == pg.MOUSEBUTTONDOWN:
                    x, y = event.pos
                    if 126 <= x <= 527 and 500 <= y <= 611:
                        game = False
                        Menu(vol0, vol1)
                    elif 627 <= x <= 1028 and 500 <= y <= 611:
                        prestart = True
                elif prestart and event.type == pg.MOUSEBUTTONDOWN:
                    best_score = sorted(get_user_best_score(user_id))[2]
                    ...
                    win = pg.mixer.Sound(os.path.join('data/AsteroidRain/music', 'lus.wav'))
                    win.set_volume(0.4 * vol1)
                    boom = pg.mixer.Sound(os.path.join('data/AsteroidRain/music', 'boom.wav'))
                    pg.mixer.music.load(os.path.join('data/AsteroidRain/music', 'fon_1.wav'))
                    pg.mixer.music.set_volume(0.25 * vol0)
                    pg.mixer.music.play(loops=-1)
                    net0 = pg.mixer.Sound(os.path.join('data/AsteroidRain/music', 'bzz.wav'))
                    net1 = pg.mixer.Sound(os.path.join('data/AsteroidRain/music', 'bipbip.wav'))
            ...
            if prestart:
                ...
                font = pg.font.Font("data/AsteroidRain/font/font0.ttf", 70)
                ...
                font = pg.font.Font("data/AsteroidRain/font/Prime-Regular.ttf", 50)
                ...
                font = pg.font.Font("data/AsteroidRain/font/font0.ttf", 50)
                ...
                if ttok == 0:
                    ...
                    if best_score[1] < score:
                        font = pg.font.Font("data/AsteroidRain/font/ofont.ru_Pobeda.ttf", 100)
                        text = font.render("НОВЫЙ РЕКОРД!", True,
                                           (150, 255, 26))
                        screen.blit(text, ((1155 - text.get_width()) // 2, 0))
                    font = pg.font.Font("data/AsteroidRain/font/Prime-Regular.ttf", 60)
                    ...
                    font = pg.font.Font("data/AsteroidRain/font/ofont.ru_Pobeda.ttf", 40)
                    ...
                    font = pg.font.Font("data/AsteroidRain/font/ofont.ru_Pobeda.ttf", 80)
                    text = font.render("Пример", True,
                                       (150, 255, 26))
                    screen.blit(text, (250, 167))

                    font = pg.font.Font("data/AsteroidRain/font/font0.ttf", 90)
                    ...
                    screen.blit(text, (
                        (
                                but_star.get_width() - text.get_width()) // 2 + 1155 // 2 - 50 - but_star.get_width(),
                        510))
                    ...
                    text = font.render("Ещё разок!", True,
                                       (150, 255, 26))
                    screen.blit(text, (
                        1155 // 2 + 50 + (but_star.get_width() - text.get_width()) // 2, 510))
                    self.screenn = screen.copy()
                if timer_over > 0:
                    net0 = pg.mixer.Sound(os.path.join('data/AsteroidRain/music', 'm_win.wav'))
                    net0.play(0)
                    timer_over -= clock.tick()
                    self.new_record = pg.Surface(screen.get_size())
                    self.new_record.blit(screen, [0, 0])
                    self.new_record.blit(new_record,
                                         [(1155 - new_record.get_width()) // 2,
                                          (650 - new_record.get_height()) // 2])
...
class Table:
    def __init__(self):
        self.rect = pg.rect.Rect((0, 0, 400, 500))
        ...
        self.scroll_area = pg.sprite.Sprite(self.draw_group)
        self.scroll_arrow_up = pg.sprite.Sprite(self.draw_group)
        self.scroll_arrow_down = pg.sprite.Sprite(self.draw_group)
        self.scroll_box = pg.sprite.Sprite(self.draw_group)

    def load_sprites(self):
        self.scroll_area.image = pg.Surface((16, self.rect.height))
        ...
        sfc = pg.Surface((16, 16))
        sfc.fill((241, 241, 241))
        pg.draw.polygon(sfc, (80, 80, 80), [(8, 8), (5, 11), (11, 11)])
        ...
        sfc = pg.Surface((16, 16))
        sfc.fill((241, 241, 241))
        pg.draw.polygon(sfc, (80, 80, 80), [(8, 8), (11, 5), (5, 5)])
        ...
        self.scroll_box.image = pg.Surface((14, self.rect.height ** 2 / self.max_y))
    ...
    def draw(self, screen: pg.Surface, width=1):
    ...
    def set_widget(self, row, col, widget: pg.Surface):
    ...
    def scroll(self, ev: pg.event.Event):
        if ev.type == pg.MOUSEBUTTONDOWN:
            point = (ev.pos[0] - self.rect.x, ev.pos[1] - self.rect.y)
            if self.scroll_arrow_up.rect.collidepoint(point):
                self.is_down = True
            elif self.scroll_arrow_down.rect.collidepoint(point):
                self.is_up = True
            elif self.scroll_box.rect.collidepoint(point):
                self.is_scroll = True
                self.scroll_y = ev.pos[1]
        if ev.type == pg.MOUSEMOTION and self.is_scroll:
...
class Puzzle(pg.sprite.Sprite):
    def __init__(self, image: pg.Surface, n0, n1):
        super().__init__()
        self.image = image
        self.rect = pg.rect.Rect(0, 0, 100, 100)
        self.j, self.i = n0, n1
...
class PuzzleGame:
    def __init__(self, screen: pg.Surface, user_id, vol0, vol1):
        self.screen = screen
        self.user_id = user_id
        self.vol0 = vol1
        self.vol1 = vol1
        ...
        self.im = pg.transform.scale(load_image('Paint', 'Repka.jpg'), (400, 400))
        self.images = pg.sprite.Group()
        data = []
        box = []
        font = pg.font.Font("data/Premenu/font/comici.ttf", 36)
        for i in range(4):
            for j in range(4):
                surface = pg.Surface((100, 100))
                surface.blit(self.im, (0, 0), pg.rect.Rect(i * 100, j * 100, 100, 100))
            ...
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    ...
                    for spr in self.images:
                        if spr.update(event):
                            break
                    if ButtonBacktoMenu.hover_check(event.pos):
                        Menu(self.vol0, self.vol1)
                elif event.type == pg.MOUSEMOTION:
...
class opt:
    def __init__(self, vol0, vol1):
        ...
        screen = pg.display.set_mode((1155, 650))

        menu_opt_pct = load_image('Options', "menu_options3.png")

        but0 = load_image('Options', "but_pro.png")
        rect00 = pg.Rect(220 + vol0 * (1155 - 480), 260, 40, 60)
        rect01 = pg.Rect(220, 60, 140, 140)
        rect_line00 = pg.Rect(220, 285, 1155 - 440, 12)

        rect10 = pg.Rect(220 + vol1 * (1155 - 480), 560, 40, 60)
        rect11 = pg.Rect(220, 360, 140, 140)
        rect_line10 = pg.Rect(220, 585, 1155 - 440, 12)
        x0, y0 = 0, 0
        x1, y1 = 0, 0
        tok0 = 1
        tok1 = 1
        gal = load_image('Options', "YES.png")
        gal = pg.transform.scale(gal, (201, 121))
        ...
        while True:
            ...
            pg.draw.rect(screen, (0, 255, 0), rect_line00)
            pg.draw.rect(screen, (0, 255, 0), rect_line10)
            # pg.draw.line(screen, (0, 0, 0), [220,290], [1155 - 220, 290], 12)
            pg.draw.line(screen, (255, 255, 255), [220, 290], [1155 - 221, 290])
            pg.draw.line(screen, (255, 255, 255), [220, 590], [1155 - 221, 590])
            font = pg.font.Font("data/Options/font/norm.ttf", 60)
            if rect00.x - 220:
                text = font.render(f"{int(100 * (rect00.x - 220) / (1155 - 480))}%", True,
                                   (255, 255, 255))
            else:
            ...
            if rect10.x - 220:
                text = font.render(f"{int(100 * (rect10.x - 220) / (1155 - 480))}%", True,
                                   (255, 255, 255))
            else:
                text = font.render("0%", True, (255, 255, 255))
            screen.blit(text, [934 - text.get_width(), 500])

            # pg.draw.rect(screen, (0, 0, 0), rect00)
            screen.blit(but0, [rect00.x, rect00.y])
            # pg.draw.rect(screen, (0, 0, 0), rect10)
            screen.blit(but0, [rect10.x, rect10.y])

            font = pg.font.Font("data/Options/font/norm.ttf", 70)
...
class Menu:
    def change_background(self, name):
        ...
        image.set_alpha(240)
        # здесь было закомент
        # image = pg.transform.scale(image, (1155, 650))
        alpha_image = load_image('Menu', name).convert()
        # здесь было закомент
        # alpha_image = pg.transform.scale(alpha_image, (1155, 650))
        ...
        self.background.blit(image, (int(self.x * 0.38) + n, 0),
                             (int(self.x * 0.38) + n, 0, self.x, 750))
        self.background.blit(alpha_image, (int(self.x * 0.38) - n, 0),
                             (int(self.x * 0.38) - n, 0, self.x, 750))
    ...
    def load_premenu(self):
        self.text = ''
        self.name = "math.png"
        self.color_inactive = pg.Color('white')
        self.color = self.color_inactive
        self.button = load_image('Premenu', "but.png")
        self.button = pg.transform.scale(self.button, (
            int(self.button.get_width() * 1.5), int(self.button.get_height() * 1.5)))
        self.font = pg.font.Font("data/Premenu/font/comici.ttf", 60)
        ...
        self.font = pg.font.Font("data/Premenu/font/comici.ttf", 36)
    ...
    def __init__(self, vol0, vol1):
        pg.display.set_caption('АРИФМАСТЕР')
    ...
    %
        ...
        self.screen = pg.display.set_mode((int(650 * 1.777777777), int(650)))
        pg.display.set_icon(pg.image.load("math.png"))
        ...
        self.progress_bar = pg.transform.scale(self.progress_bar, (212, 32))
        self.star = pg.transform.scale(self.star, (71, 71))
        self.frame = pg.transform.scale(self.frame, (101, 101))
        self.photo = pg.transform.scale(self.photo, (101, 101))
        self.MENU = True
        self.PREMENU = False
        if get_users() == []:
            self.PREMENU = True
    ...
    def blit_lineedit(self, text, screen, color):
        font = pg.font.Font("data/Premenu/font/comici.ttf", 60)
        w = 430
        h = 80
        input_box = pg.Rect((1155 - w) // 2, 100, w, h)
        ...
        pg.draw.rect(screen, color, input_box, 4)
        ...
        while start:
            ...
            for event in pg.event.get():
                ...
                if event.type == pg.MOUSEMOTION:
                    ...
                    ButtonChangeClient.update(event.pos)
                    ButtonOptions.update(event.pos)
                if self.PROFIL and event.type == pg.MOUSEBUTTONDOWN:
                    if ButtonBacktoMenu.hover_check(event.pos):
                        self.PROFIL = False
                        self.MENU = True
                if self.MENU and event.type == pg.MOUSEBUTTONDOWN:
                    x, y = event.pos
                    if self.MENU:
                        ...
                        if ButtonRaitingTable.hover_check(event.pos):
                            RatingWindow(self.screen, user_id_glob, self.vol0, self.vol1)

                        if ButtonChangeClient.hover_check(event.pos):
                            self.ex.show()

                        if ButtonOptions.hover_check(event.pos):
                            opt(self.vol0, self.vol1)
                if self.PREMENU:
                    if event.type == pg.MOUSEBUTTONDOWN:
                        x, y = event.pos
                        if (1155 - self.button.get_width()) // 2 <= x <= (
                                1155 - self.button.get_width()) // 2 + self.button.get_width():
                            if 400 <= y <= 400 + self.button.get_height():
                                if 3 < len(self.text) < 13:
                    ...
                    if event.type == pg.KEYDOWN:
                        if event.key == pg.K_RETURN:
                            ...
                            if self.text != '':
                                if (self.text + event.unicode).istitle() and len(
                                        self.text) <= 12 and (
            ...
            if self.PREMENU:
                ...
                self.screen.blit(self.txt_surface4,
                                 [(1155 - self.txt_surface4.get_width()) // 2, 220])
            ...
            if self.MENU:
                ...
                ButtonRaitingTable.draw(self.screen)
                ButtonChangeClient.draw(self.screen)
                ButtonOptions.draw(self.screen)

                # <--ButtonProfile
                # BackGround
                self.screen.blit(self.button0, (800, 20))
                pg.draw.line(self.screen, (255, 255, 255), (905, 70), (1115, 70), 2)
                # Progress bur
                pg.draw.polygon(self.screen, (255, 255, 255), [[905, 80], [1116, 80],
                                                               [1116, 111], [905, 111]])
                if 212 * (get_user_have_score(self.user_id) / get_user_must_score(self.user_id)):
                    pg.draw.rect(self.screen, (244, 191, 0), [905, 80, 212 * (
                            get_user_have_score(self.user_id) / get_user_must_score(
                        self.user_id)), 32])
                self.screen.blit(self.progress_bar, [905, 80])
                level = f"{get_user_have_score(self.user_id)}/{get_user_must_score(self.user_id)}"
                font = pg.font.Font("data/Menu/font/Prime-Regular.ttf", 30)
                ...
                font = pg.font.Font("data/Menu/font/comici.ttf", 30)
                ...
                font = pg.font.Font("data/Menu/font/Prime-Regular.ttf", 30)
                text = font.render(level, True, (0, 0, 0))
                self.screen.blit(text, [770 + (71 - text.get_width()) // 2,
                                        80 + (71 - text.get_height()) // 2])
                # -->
            if self.PROFIL:
                self.client_profil.start(
                    get_user_have_score(self.user_id) / get_user_must_score(self.user_id))
                name = get_user_name(self.user_id)
                font = pg.font.Font("data/Profile/font/comici.ttf", 40)
                text = font.render(name, True, (255, 255, 255))
                ...
                font = pg.font.Font("data/Profile/font/norm.ttf", 40)
                text = font.render("Обучение: " + str(box[0][1]), True, (255, 255, 255))
                self.screen.blit(text, [565, 260])

                font = pg.font.Font("data/Profile/font/AngryBirds.ttf", 40)
                num = "-"
                if len(box) >= 2:
                    num = str(box[1][1])
                ...
                font = pg.font.Font("data/Profile/font/font0.ttf", 40)
                num = "-"
                if len(box) >= 3:
                    num = str(box[2][1])
                ...
                font = pg.font.Font("data/Profile/font/Prime-Regular.ttf", 70)
                text = font.render(level, True, (0, 0, 0))
                self.screen.blit(text, [280 + (211 - text.get_width()) // 2, 373])

                level = f"{get_user_have_score(self.user_id)}/{get_user_must_score(self.user_id)}"
                # не больше 12 симвoлов
                font = pg.font.Font("data/Profile/font/Prime-Regular.ttf", 50)
                text = font.render(level, True, (0, 0, 0))
...
if __name__ == '__main__':
    ...
    ButtonBacktoMenu = GPushButton("")
    ButtonBacktoMenu.set_image("data/Buttons/quit.png")
    ButtonBacktoMenu.set_position((0, 0))
    ButtonBacktoMenu.set_size((86, 81))

    ButtonOptions = GPushButton("")
    ButtonOptions.set_image("data/Buttons/but_opt.png")
    ButtonOptions.set_position((1030, 540))
    ButtonOptions.set_size((91, 93))
    ButtonOptions.hover_AnimationChangeSize(-6, -6)

    ButtonChangeClient = GPushButton("")
    ButtonChangeClient.set_image("data/Buttons/but_chng.png")
    ButtonChangeClient.set_position((1065, 476))
    ButtonChangeClient.set_size((55, 55))
    ButtonChangeClient.hover_AnimationChangeSize(-5, -5)

    ButtonRaitingTable = GPushButton("Рейтинг")
    ButtonRaitingTable.set_image("data/Buttons/but_table.png")
    ButtonRaitingTable.set_position((965, 130))
    ButtonRaitingTable.set_size((158, 44))
    ButtonRaitingTable.set_font("data/Menu/font/norm.ttf")
    ButtonRaitingTable.hover_AnimationChangeSize(-10, -6)
    ButtonRaitingTable.hover_AnimationOfText(textColor=(230, 230, 230), fontSize=30)
    ButtonRaitingTable.set_fontSize(30)
    ButtonRaitingTable.set_textColor((255, 255, 255))
    ButtonRaitingTable.update((-1, -1))
